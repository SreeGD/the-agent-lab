...
- **Wandering** — agent loops without converging on an answer (mitigate: recursion limit, explicit "you have N steps left" in the system prompt)
- **Bad first step locks in bad path** — once the model commits to an approach, it rarely backtracks (mitigate: pair with Reflection)

**Examples:** customer support chatbot, calculator-with-tools, Q&A over a knowledge base, conversational coding assistant (Cursor-style)
...
**Don't use when:** transactional Q&A, low-stakes chat, or anywhere the user expects a one-shot response.
...
- **Iteration explosion** — critic always finds *something* to nitpick (mitigate: hard iteration budget, "approve" if criteria are met)
- **Cost runaway** — 2-5× the base cost; track tokens (mitigate: cache the static parts of writer/critic prompts; use smaller critic model)
- **Echoing instead of revising** — writer regenerates similar drafts each round (mitigate: explicitly feed back specific feedback, not just "try again")

**Examples:** code review agents (writer + linter-critic), legal/medical draft generation, marketing copy with brand-voice critic, generated technical documentation with accuracy critic

**Variant — Reflexion:** like reflection, but the agent maintains *persistent memory* of past failures across runs. Better for agents that retry the same task category (e.g., a debugging agent that remembers "I tried fix X last time and it didn't work").
...
**Use when:** the task is genuinely multi-step, the steps are independent or have known dependencies, and a small model can do the planning while a larger model does the execution.

**Don't use when:** single-step tasks, conversational chat, or anything where the user's intent is ambiguous (you can't plan what you don't understand).

**Common problems:**
- **Bad plan poisons the whole run** — if the planner mis-decomposes, no amount of executor smartness recovers (mitigate: planner outputs a typed `Plan` schema with required fields; validate before executing; pair with Reflection on the plan itself)
- **No replanning when reality differs** — executor hits something the planner didn't anticipate (mitigate: add a "should I replan?" gate after each step, or switch to ReWOO if you genuinely can't replan)
- **Sub-step context loss** — step 3 doesn't know what step 1 produced (mitigate: pass prior step results as context into each execution)
- **Aggregator hallucination** — final aggregator summarizes things the steps didn't produce (mitigate: structured aggregator output; faithfulness check vs. step results)

**Examples:** research agents (search → synthesize → write → cite), coding workflows (scaffold → implement → test → docs), data pipelines (extract → transform → load → verify), report generation (gather → outline → write sections → assemble)

**Variant — ReWOO (Reasoning WithOut Observation):** Plan-and-Execute with **no replanning step**. One big upfront plan, executed without feedback loops. Cheaper but more brittle. Use when steps are mechanical and don't need observation.

### Other patterns worth knowing (one-liner each)

- **Chain of Thought (CoT)** — agent writes its reasoning out loud before the answer; structured-output trick: put a `reasoning: str` field *first* in your Pydantic model
- **Self-Consistency** — run the same prompt N times in parallel; take the majority answer. Reduces variance on hard reasoning tasks at N× cost
- **Tree of Thoughts (ToT)** — branching exploration of reasoning paths; like search with the LLM as both move-generator and evaluator. Useful for puzzles, search-heavy problems
- **Self-Ask** — agent explicitly asks itself sub-questions before the final answer. Cheaper, lighter cousin of Plan-and-Execute
- **Constitutional AI** — rule-based self-critique using a fixed "constitution" of principles. Anthropic's specific flavor of Reflection
- **Verifier / Judge** — an external LLM or program validates the output. Used in eval pipelines and as the output-side of guardrails (see `safe_rag.py`'s faithfulness judge)
- **Tool-Use w/ Retry** — a baby version of Reflection: try a tool call; if it fails, retry with the error appended to context
- **Multi-agent (Supervisor / Swarm)** — N specialist agents coordinated by a router. Different problem from the three above — it's *between* agents, not *within* one (covered in Session 3)
...
2. **Hallucination compounds** — a bad first step poisons later steps. Add a verifier/grounding check at the output.
3. **Loop budgets are non-optional** — every iterative pattern needs a max-iterations safety floor. Without it, you will get bills you can't explain.
4. **Eval is harder than building** — for any pattern beyond ReAct, you need a way to measure whether the added complexity is actually helping. See Session 10.
5. **Single-model self-critique is weak** — using the same model as both producer and critic gets diminishing returns fast. Use a different model, or at least different system prompts.
6. **The "prompt loop" trap** — when an agent isn't working, the temptation is to add more prompt instructions. Often the answer is *less* prompt + a better pattern.
