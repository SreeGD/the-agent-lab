...
| File | Role |
|---|---|
| [`33_ux_audit_helper_ollama.py`](../ollama/33_ux_audit_helper_ollama.py) | The audit tool. Input: feature description. Output: structured `UXAudit{should_be_ai, ai_scope, required_ux_patterns, trust_calibration, failure_ux, risks, recommendations, rollout_guidance}` via `with_structured_output`. Four demo features hit different shapes of the design space. |
...
**Ollama note on slow_response:** with Ollama running locally, inference speed depends heavily on hardware. On slower machines, responses may exceed 3s even for short outputs. Always implement STREAMING_FEEDBACK for any Ollama-backed feature — users need to see tokens arriving to know inference is working, not hanging.
...
```bash
python ollama/33_ux_audit_helper_ollama.py
```

~2-3 minutes (4 features × 1 structured LLM call each). No API cost — Ollama runs locally.
...
A: Make the bar simple: any feature gated by a leadership review must come with the audit attached. Once the audit is required for sign-off, teams will run it. The barrier is usually social, not technical — the tool is `python ollama/33_ux_audit_helper_ollama.py` and a string.

**Q: Can I generate the audit in real time as the user describes the feature in a meeting?**

A: Yes — the tool runs in ~30s per feature with Ollama locally (varies by hardware). Some teams put it on a shared screen during product reviews. The shared output becomes the discussion artifact for the next 30 minutes. *"The audit says X — do we agree or do we have a constraint?"* is a much better meeting than *"what do you all think?"*
