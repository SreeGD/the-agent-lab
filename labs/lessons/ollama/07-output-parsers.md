...
| File | Role |
|---|---|
| [`07_output_parsers_ollama.py`](../ollama/07_output_parsers_ollama.py) | Six parsers walked through against one movie review |
...
```bash
python ollama/07_output_parsers_ollama.py
```
...
| Provider support | Any LLM (works via prompting) | Requires tool-calling (Ollama + llama3.2, OpenAI, Gemini) |
...
1. **Replace `StrOutputParser` with `JsonOutputParser`** in `02_lcel_chain_ollama.py` — the chain now returns a dict if the prompt asks for JSON.
