...
| File | Role |
|---|---|
| [`03_agent_manual_ollama.py`](../ollama/03_agent_manual_ollama.py) | The manual tool loop — hand-written `while True` |
...
The loop runs until Llama 3.2 stops asking for tools (i.e., produces an `AIMessage` with `.content` and no `tool_calls`).
...
### Manual loop (`03_agent_manual_ollama.py`)
...
```bash
# Manual loop:
python ollama/03_agent_manual_ollama.py
...
1. **Break a tool intentionally** — `raise ValueError("nope")` in `add`. Watch Llama 3.2 get the error as a `ToolMessage` and recover.
2. **Unanswerable question** — *"What's the weather in Tokyo?"* with no weather tool. The model refuses instead of hallucinating.
3. **Sequential dependency** — add a `multiply(a, b)` tool, ask *"What is (47+158) × 3?"*. Watch a 3-turn loop: the model calls `add`, waits for the result, then calls `multiply(a=205, b=3)`.
4. **Compare manual vs framework** — diff `03_agent_manual_ollama.py` against `03_agent_framework.py`. Notice how much the loop logic shrinks.
...
**Q: Why does the manual `03_agent_manual_ollama.py` exist if `03_agent_framework.py` does the same thing in 5 lines?**
...
A: Yes — by default Llama 3.2 will ask for clarification when a tool's args aren't clear. You can also engineer prompts ("Ask me for missing info before calling tools") to bias this behavior.
...
Without a limit, a bug can run indefinitely.
