...
> **Provider variant — Ollama (`llama3.2`)** This is the Ollama version of this lesson. The logic, patterns, and exercises are identical to the Anthropic version. What differs: import is `langchain_ollama.ChatOllama`, model is `llama3.2`, no API key is needed (Ollama runs locally), and `ollama serve` must be running. Code file: `labs/ollama/34_farm_planner_engine_ollama.py`.
>
> **Important Ollama note:** The original engine uses native Anthropic SDK with `cache_control: ephemeral` on the system prompt. The Ollama version uses `ChatOllama` with a plain-string `SystemMessage` — there is no prompt caching mechanism in Ollama. The 7K-token knowledge base is re-billed at full inference cost on every call. For repeated planning runs, consider pre-loading the model or batching requests to amortize startup overhead.
...
| File | Role |
|---|---|
| [`34_farm_planner_engine_ollama.py`](../ollama/34_farm_planner_engine_ollama.py) | **Pure Python engine** — no UI imports. All business logic: profile + plan I/O, LLM call with knowledge-base-grounded system prompt using `ChatOllama`, sustainability scoring (deterministic), markdown + PDF rendering. Reusable from any UI. |
| [`34_farm_planner_ui.py`](../34_farm_planner_ui.py) | **Streamlit UI** — multi-page form-driven app. Calls engine functions; no business logic of its own. Swap the engine import to use the Ollama version. |
| [`34_farm_planner_api.py`](../34_farm_planner_api.py) | **FastAPI stub** — REST endpoints wrapping the same engine. Proves the future React-frontend migration path is a swap, not a rewrite. |
...
The advisor fixes all three by embedding the [Telangana Knowledge Base](../agritech/telangana_knowledge_base.md) into the system prompt. Same LLM, dramatically better output.
...
└──────────────────────────────────┬─────────────────────────────────────┘
                                   ...
                                   ▼
┌────────────────────────────────────────────────────────────────────────┐
│         ENGINE (34_farm_planner_engine_ollama.py)                      │
│                                                                        │
│  generate_farm_plan(profile, goals) ─┐                                 │
│                                      ▼                                 │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │  System prompt (plain string — no cache_control for Ollama)     │  │
...
│  └──────────────────────────────────────────────────────────────────┘  │
│                              ↓                                         │
│  ChatOllama(model="llama3.2").with_structured_output(FarmPlan)         │
│  max_tokens=8192, timeout=600s, retry x3 with exponential backoff      │
│                              ↓                                         │
│  FarmPlan (Pydantic):                                                  │
...
def _build_system_prompt() -> str:
    ...
    # Plain string system message — no cache_control for Ollama
    return SYSTEM_PROMPT_TEMPLATE.format(knowledge_base=knowledge)
```

The 7K-token knowledge base is inlined into the system prompt as a plain string. Unlike the Anthropic version which uses `cache_control: ephemeral`, Ollama has no prompt caching mechanism — the full knowledge base is processed on every call. On slower hardware this adds meaningful inference time; on Apple Silicon or a capable GPU it remains practical.

**Performance note:** the `with_structured_output()` + `cache_control` combination in `langchain-anthropic` causes silent hangs (the source lesson documents this). This issue does NOT apply to the Ollama version — plain-string `SystemMessage` with `ChatOllama` works cleanly with `with_structured_output()`.
...
**Ollama note:** llama3.2 is smaller than Sonnet and may have lower structured output fill rates on complex nested schemas. The multi-call approach (see below) mitigates this by breaking the schema into smaller chunks per call.

### 3. The engine is a swap point, not a tangle

```python
# Public API of 34_farm_planner_engine_ollama.py
...
### 5. The multi-call approach

For large output schemas, a single LLM call can hit token limits and drop fields. The engine ships `generate_farm_plan_multicall` — three sequential structured-output calls instead of one:
...
Each call passes prior calls' output as context, so the LLM stays consistent across sections. Each fits comfortably in 4-5K output tokens (no field-dropping). **`generate_farm_plan(profile, goals, use_multicall=True)` is the default**; pass `use_multicall=False` to revert to the single big call.

This is especially important with `llama3.2` which has a smaller context window than Sonnet — splitting into 3 calls is effectively required for complete plan generation.

### 6. The Streamlit → FastAPI migration path
...
Swap the engine import in `34_farm_planner_ui.py` to `import ollama.34_farm_planner_engine_ollama as engine`.

Browser opens at `http://localhost:8501`. Pick `sample_suryapet` from the Home page → look at the auto-loaded profile → go to Goals & Constraints → tick `Include dairy` and `Include apiary` → Generate Plan → watch the spinner (~30-180s depending on hardware) → View Plan → explore tabs → Download as PDF.

### Engine sanity check

```bash
./.venv/bin/python ollama/34_farm_planner_engine_ollama.py
```
...
The committed [`farm_plans/sample_plan_suryapet.json`](../farm_plans/sample_plan_suryapet.json) is an **actual saved plan** generated against the Suryapet sample profile. Highlights below; the full JSON has every field populated.

**Runtime:** ~3-5 minutes locally on Apple Silicon (system prompt embedding the 7K-token KB + 8K-token structured output). No API cost — Ollama runs locally. The engine's timeout is set to 600s for this reason.
...
> *Live output varies per run. With llama3.2, plan depth may differ from Sonnet-generated plans — the knowledge base + schema enforce the structure, but reasoning depth scales with model capability.*
...
Network issues and local Ollama overload (queue full) happen. The engine's retry loop with exponential backoff (0s, 10s, 30s, 60s between retries) handles transients without manual intervention. Production should also:
- Surface "retrying..." to the user (Session 21 UX pattern)
...
The knowledge base explicitly lists varieties (Thailand Lemon, Fuerte avocado, Bhagwa pomegranate...) and suppliers (SKLTSHU, Deccan Exotics, Indo Israel Avocado). The system prompt instructs the LLM to use these — not invent. With llama3.2, you may see slightly more drift than with Sonnet; the disclaimer + KVK escalation note is the safety net.
...
With llama3.2, complex schemas push the model toward field-dropping more quickly than Sonnet. The multi-call approach (3 sequential calls, each with a subset of the schema) is the primary mitigation. Larger schemas → more calls, each handling a focused subset.

**Q: Why timeout=600s?**
The full FarmPlan output is ~3K-5K output tokens (60+ fields total, including nested lists). With local inference on moderate hardware, that can take several minutes. The 600s timeout gives headroom for slower machines and warm-up time.
...
With Ollama, the LLM runs entirely locally — inference is already offline. The advisor does NOT need internet access for plan generation. The UI is local, the JSON state is local, and the LLM call is local. This makes it genuinely usable in field conditions where connectivity is limited.
