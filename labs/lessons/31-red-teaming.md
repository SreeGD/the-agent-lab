...
| File | Role |
|---|---|
| [`31_red_teaming.py`](../31_red_teaming.py) | The full harness: two chatbot targets (vulnerable vs 3-layer defended), an attack catalog of 12 across 6 categories, an LLM-as-judge for "did the attack succeed", a scoreboard, an auto-generated model card, and an audit log template. |
...
Each layer adds latency and cost. Layer 1 is essentially free (regex). Layer 2 is a system-prompt change (still free). Layer 3 is one Haiku call per response (~$0.0002). The three layers together cost ~$0.0005 extra per response — pennies for the safety floor they provide.
...
This is the *belt-and-suspenders* layer. A small, fast model (Haiku) reads the candidate response and checks for the failure modes that matter. If any fire, the response is replaced with a `[BLOCKED]` message.
...
For a 12-attack catalog × 2 targets, this is 24 judge calls — cheap, especially with Haiku as judge.
...
```
cd labs
./.venv/bin/python 31_red_teaming.py
```
...
- The LLM-as-judge validator (Haiku) shares blind spots with the target (Sonnet) — adversarial outputs that fool both pass through
...
- LLM-as-judge output validation (Haiku)
...
- Even Anthropic's own red team can't get residual to 0
...
**Q: Why did Sonnet 4.6 block all 12 attacks even on the "vulnerable" target in the first run?**
...
- For high-stakes settings, use *two different model families* (e.g., Haiku as validator + an OSS model like LlamaGuard) — adversarial outputs that fool both are much rarer

**Q: How does this interact with prompt caching (Session 15)?**
The system prompts (vulnerable, hardened, structured) are stable across requests — perfect cache candidates. Add `cache_control: ephemeral` and Layer 2's overhead drops to ~$0 after the first call. The Layer 3 validator's system prompt is also stable; same treatment.

**Q: Can the LLM-as-judge be the same model as the target?**
You CAN, but you SHOULDN'T. Same-family models share blind spots — an adversarial output that fools the generator also tends to fool the judge. Use different families when possible. In this lab Sonnet generates and Haiku judges (same family, but different tier — partial mitigation).
...
**Q: Does Anthropic publish their internal red-team findings?**
Yes — Anthropic publishes "model cards" and "responsible scaling" reports for each major model release, including known failure modes and the mitigations applied. These are useful templates for your own model cards. Their research org also publishes papers on emergent attack patterns — worth subscribing to their feed.
