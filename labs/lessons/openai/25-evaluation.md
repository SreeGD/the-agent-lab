...
| [`25_evaluation_openai.py`](../../openai/25_evaluation_openai.py) | 4 metrics (faithfulness, answer relevance, context precision, context recall) as LLM-as-judge calls. Runs 3 RAG variants (dense / hybrid / CRAG) over the golden set, prints a comparison table, then a regression check. |
...
All four return a single float in `[0.0, 1.0]`, judged by GPT-4o with structured output. Read each prompt — that's where the metric is *defined*.
...
Why use GPT-4o as judge? Two reasons:
...
```
cd labs
./.venv/bin/python openai/25_evaluation_openai.py
```

Takes ~3-5 minutes (120-ish LLM calls). Cost ~$0.30-0.50 with gpt-4o. If you want it faster + cheaper, switch the judge model to `gpt-4o-mini` — almost identical numbers at ~$0.05.
...
For ternary/binary classification (which the metrics essentially are), gpt-4o-mini is plenty. gpt-4o is overkill. Try both and compare scores on your golden set — if they agree within ±0.05, ship the cheap one.
...
- run: python labs/openai/25_evaluation_openai.py --threshold OVERALL=0.75
...
2. **Switch the judge to gpt-4o-mini.** Time the run, compare scores. Are any metric averages > 0.05 apart? If not, ship the cheap one.
