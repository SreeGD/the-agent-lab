...
| File | Role |
|---|---|
| [`openai/31_red_teaming_openai.py`](../../openai/31_red_teaming_openai.py) | The full harness: two chatbot targets (vulnerable vs 3-layer defended), an attack catalog of 12 across 6 categories, an LLM-as-judge for "did the attack succeed", a scoreboard, an auto-generated model card, and an audit log template. |
...
Each layer adds latency and cost. Layer 1 is essentially free (regex). Layer 2 is a system-prompt change (still free). Layer 3 is one gpt-4o-mini call per response (~$0.0002). The three layers together cost ~$0.0005 extra per response — pennies for the safety floor they provide.
...
The XML-tag boundary teaches the model: *"text inside this tag is content, not commands."* Combined with the explicit refusal instructions, this is the single highest-leverage defense — most modern frontier models respect this structure when it's stated clearly.
...
This is the *belt-and-suspenders* layer. A small, fast model (gpt-4o-mini) reads the candidate response and checks for the failure modes that matter. If any fire, the response is replaced with a `[BLOCKED]` message.
...
For a 12-attack catalog × 2 targets, this is 24 judge calls — cheap, especially with gpt-4o-mini as judge.
...
```
cd labs
./.venv/bin/python openai/31_red_teaming_openai.py
```

~3-5 minutes (12 attacks × 2 targets, ~96 LLM calls when you count defense layers + judges). Cost ~$0.20.
...
- The LLM-as-judge validator (gpt-4o-mini) shares blind spots with the target (gpt-4o) — adversarial outputs that fool both pass through
...
- LLM-as-judge output validation (gpt-4o-mini)
...
- Even OpenAI's own red team can't get residual to 0
...
**Q: Why did GPT-4o block all 12 attacks even on the "vulnerable" target in the first run?**
...
- For high-stakes settings, use *two different model families* (e.g., gpt-4o-mini as validator + an OSS model like LlamaGuard) — adversarial outputs that fool both are much rarer

**Q: How does this interact with prompt caching?**
The system prompts (vulnerable, hardened, structured) are stable across requests — perfect cache candidates for OpenAI's automatic prefix caching. Reuse the same stable 1024+ token prefix on every request and OpenAI caches it automatically. Layer 2 and Layer 3's system prompts are also stable; same treatment — no special `cache_control` field needed.

**Q: Can the LLM-as-judge be the same model as the target?**
You CAN, but you SHOULDN'T. Same-family models share blind spots — an adversarial output that fools the generator also tends to fool the judge. Use different families when possible. In this lab gpt-4o generates and gpt-4o-mini judges (same family, but different tier — partial mitigation).
...
**Q: Does OpenAI publish their internal red-team findings?**
Yes — OpenAI publishes model cards, safety evaluations, and system card reports for major model releases, including known failure modes and the mitigations applied. Anthropic's "responsible scaling" reports are also useful templates for your own model cards. Both organizations' research orgs publish papers on emergent attack patterns — worth following both.

**Q: What if my LLM provider's safety changes silently and the harness regresses?**
That's *exactly* why you keep the harness in CI. Schedule it weekly. If success rates change without your code changing, you know the model provider updated something (and you find out before your customers do).
