...
| File | Role |
|---|---|
| [`02_lcel_chain_openai.py`](../../openai/02_lcel_chain_openai.py) | Three-stage LCEL chain: prompt → model → parser |
...
def explain(topic: str) -> str:
    ...
    response = openai_client.chat.completions.create(       # 2. raw API call
        model="gpt-4o",
        messages=[{"role": "user", "content": prompt_text}],
    )
    text = response.choices[0].message.content              # 3. manual unwrap
    return text
```
...
python openai/02_lcel_chain_openai.py
```
...
| `model` | the messages | an `AIMessage` from GPT-4o |
...
A: `ChatPromptTemplate` produces a **list of messages** (system + human + ...); `PromptTemplate` produces a **single string**. For chat-style models like GPT-4o, use `ChatPromptTemplate`. For completion-style models, use `PromptTemplate`.
