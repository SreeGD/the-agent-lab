...
```
   ...
   USER REQUEST  ─────►   │  pre-flight (cheap)   │ ──── PII regex → if hit, DENY here
                          ...
                          └───────────┬───────────┘
                                      ...
                                      ▼
                          ┌───────────────────────┐
                          │  META-JUDGE confidence│ ──── gpt-4o-mini
...
**Meta-judge confidence** comes from a *different* model (we use gpt-4o-mini) that reads the question + answer + self-confidence and scores them independently. Has its own biases, but rarely the same biases as the answerer.
...
```
cd labs
./.venv/bin/python openai/32_governance_openai.py
```
...
The meta-judge is gpt-4o-mini (~$0.0002 per call). At 1M requests/month, meta-judging every one is ~$200/mo. The information you get — the gap as a calibration signal — is worth more than that in audit-readiness alone. For cost-sensitive deployments, sample (meta-judge 10% of requests, monitor drift).

**Q: Won't the meta-judge have its own biases?**
Yes. Mitigations: (1) use a *different model family* from the answerer when possible, (2) periodically re-baseline the meta-judge against a human review panel, (3) for highest-stakes domains, use ensemble of 2-3 judges and vote.
...
`with_structured_output(AnswerWithConfidence)` returns a Pydantic instance with `answer` + `confidence` + `reasoning` populated in one call. No extra LLM round-trip. Modern OpenAI + LangChain make this clean.
