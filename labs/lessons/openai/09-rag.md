...
| File | Role |
|---|---|
| [`09_rag_openai.py`](../../openai/09_rag_openai.py) | Full 6-stage RAG pipeline over your `NOTES.md` + `LEARNINGS.md` (self-referential demo) |
...
If you ask GPT-4o *"What's the API for our internal billing service?"* it has no idea. Worse, it might hallucinate a plausible-sounding-but-wrong answer.
...
```bash
python openai/09_rag_openai.py
```
...
For one query, the full HTTP request to api.openai.com is just two messages:
...
1. **Ask a question NOT in the notes** — *"What's the weather today?"* With the strict system prompt, GPT-4o correctly says *"I don't have enough information."*
