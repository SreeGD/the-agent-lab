...
| File | Role |
|---|---|
| [`24_corrective_rag_openai.py`](../../openai/24_corrective_rag_openai.py) | A LangGraph `StateGraph` implementing the full CRAG pipeline: retrieve → grade → {use_chunks \| rewrite \| web_fallback} → answer. Three demo queries, one per branch. |
...
```
cd labs
./.venv/bin/python openai/24_corrective_rag_openai.py
```
...
- **Cheaper grader model** — `gpt-4o-mini` is plenty for binary/ternary classification. Don't pay gpt-4o prices for a yes/no judgment.
...
2. **Switch the grader to gpt-4o-mini** — change `MODEL = "gpt-4o-mini"` *just for the grader*. Time the run. Same verdicts? (Spoiler: usually yes.) Compute the cost savings.
...
Yes. Two mitigations: (1) use the cheapest model that can do the judgment — gpt-4o-mini is usually fine, (2) batch the grading into a single call ("here are 5 chunks, give me 5 verdicts") instead of one call per chunk. The latter cuts cost ~4x with minimal accuracy loss.
...
The retrieval grader prompt is the SAME for every chunk in a query — only the chunk content varies. So the system prompt + question are cacheable. With gpt-4o-mini as the grader and OpenAI's automatic prefix caching, the cost overhead of CRAG drops significantly. Worth it.
