...
- Recommend gpt-4o for everything because gpt-4o-mini didn't come to mind
...
```
   ...
   USER ─────────────────►  │   API gateway       │ ◄── auth, rate limit, TLS
                            ...
                            └──────────┬──────────┘
                                       ...
                                       ▼
                            ┌─────────────────────┐
                            │ Orchestrator        │ ◄── intent classifier (gpt-4o-mini)
   ...
   └────────┬────────┘        └────────┬────────┘      └─────────────────┘
            ...
            └──────────┬───────────────┘
                       ▼
            ┌──────────────────────┐
            │  LLM Gateway         │ ◄── model routing (gpt-4o-mini/gpt-4o),
            │  (caching, retry)    │      automatic prefix caching on stable prefix
            └──────────┬───────────┘
                       │
                       ▼
            ┌──────────────────────┐
...
| **Model strategy** | one model / per-role (gpt-4o-mini triage, gpt-4o final) / fine-tune | **Per-role with gpt-4o-mini for cheap, gpt-4o for hard** — Session 15 |
...
cost/month = tokens_in × $/M_in + tokens_out × $/M_out
```

**Example: fintech chatbot, 10M MAU**
```
10M MAU × 2 sessions/month × 5 turns/session = 100M turns/month
With per-role routing: 70% gpt-4o-mini, 30% gpt-4o
gpt-4o-mini turns:  70M × (500 input + 200 output)
              = 35B input × $0.15/M + 14B output × $0.60/M
              = $5.25K + $8.4K = $13.65K/month
gpt-4o turns: 30M × (800 input + 400 output)
              = 24B input × $2.50/M + 12B output × $10/M
              = $60K + $120K = $180K/month
Total:        ~$194K/month before caching
With automatic prefix caching (60% hit rate on system prompt): saves ~$20K
Net:          ~$174K/month
```

That's the math the helper tool walked through above. Memorize the *shape* of this calculation — interviewers will substitute different numbers but the formula stays.
...
- intent classify (gpt-4o-mini, streamed): 400ms TTFT
- retrieval (vector + BM25): 200ms
- LLM (gpt-4o, streamed): 800ms TTFT, 1500ms total for ~300 output tokens
...
If the interviewer says "p95 must be 1 second" — that math doesn't fit. You either drop gpt-4o (use gpt-4o-mini for everything), drop retrieval, or move to a faster region. Knowing the budget line-by-line is what lets you negotiate.
...
▸ LLM model selection strategy
    options: Single gpt-4o | Two-tier gpt-4o-mini+gpt-4o | Fine-tuned smaller
    RECOMMEND: Two-tier gpt-4o-mini for triage, gpt-4o for drafting
    because: At 100M turns/month, single gpt-4o costs ~$200K/mo;
             two-tier cuts ~60% by routing 70% of turns to gpt-4o-mini.
...
| OpenAI API outage | L | H | Circuit-breaker, fallback to "we're experiencing issues, escalating to agent" |
...
2. **"How would you cut cost by 50%?"** — the levers from Session 15: model selection, caching, compression, batches. Quantify each.
...
4. **"What's the most likely thing to break in production?"** — name a real failure mode (OpenAI 5xx, retrieval miss, prompt injection) and describe the detection + response.
...
```
cd labs
./.venv/bin/python openai/30_system_design_helper_openai.py
```

Takes ~30-60 seconds (three full design generations). Cost ~$0.10 with gpt-4o.
...
**Architecture (ASCII):** 16-component diagram showing API gateway → session manager → orchestrator → RAG + tool router + escalation → LLM gateway (gpt-4o-mini triage + gpt-4o drafting) → validator → audit logger.

**Capacity (key numbers, with assumptions):**
- Peak QPS: **~580** (10M MAU × 2 sessions/mo × 5 turns × 15× burst)
- Monthly LLM cost: **~$90K** (after per-role routing + automatic prefix caching)
...
1. LLM model selection → per-role (gpt-4o-mini triage + gpt-4o drafting)
...
- "How would you cut cost by 50% without degrading UX?"
...
That's the depth a real interview goes to. The tool ran in 15 seconds.
...
- **Edge / on-device** (private LLMs) — different model selection entirely, no OpenAI API
...
**Q: Will this lesson be enough to pass a staff-level AI interview at OpenAI / Google?**
