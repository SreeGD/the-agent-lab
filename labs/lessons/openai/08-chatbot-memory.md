...
| File | Role |
|---|---|
| [`08_chatbot_memory_openai.py`](../../openai/08_chatbot_memory_openai.py) | 3-turn conversation in one thread + isolated second thread |
...
```bash
python openai/08_chatbot_memory_openai.py
```
...
1. **Continue Alice's conversation** — add `turn("alice", "What was the answer again?")`. Watch GPT-4o recall `46` without re-running tools.
...
4. **Pair with caching** — add a substantial system prompt. Watch input cost stop growing linearly. This is the *complete* production chatbot architecture.
