...
| File | Role |
|---|---|
| [`10_guardrails_openai.py`](../../openai/10_guardrails_openai.py) | RAG pipeline wrapped in 3 input guardrails + 2 output guardrails, with 5 test inputs |
...
### Input guardrails (`10_guardrails_openai.py` implements three)
...
| **On-topic LLM-judge** | Cheap classifier: *"is this a LangChain/GPT-4o question?"* → one word | 1 small LLM call | refuse |

### Output guardrails (`10_guardrails_openai.py` implements two)
...
```bash
python openai/10_guardrails_openai.py
```
...
5. **Wrap `08_chatbot_memory_openai.py` with guardrails** — now you have a guardrailed stateful chatbot. (Or see [lesson 11](11-production-capstone.md) for the full composition.)
...
A: Two ways:
- **Real-time guard** — a faithfulness LLM-judge wrapping each call (what `10_guardrails_openai.py` does). Adds ~600ms + tokens.
...
- **Use a smaller model for guards** — gpt-4o-mini for guards, gpt-4o for main work; ~80% cost reduction
