...
| File | Role |
|---|---|
| [`01_model_wrapper_openai.py`](../../openai/01_model_wrapper_openai.py) | The minimal example: load env → instantiate `ChatOpenAI` → invoke → print |
...
`ChatOpenAI` (and its sibling classes `ChatAnthropic`, `ChatGoogleGenerativeAI`, `ChatOllama`, etc.) **give you the same `.invoke()` / `.stream()` / `.batch()` interface across every provider**. Swap the import + the constructor and the rest of your code is unchanged.
...
Your code
   ...
   ▼
┌──────────────────────────────────────┐
│   ChatOpenAI (model wrapper)         │  ← uniform interface
│                                      │
...
Same diagram with `ChatAnthropic` → `api.anthropic.com` → Claude. Wire details differ; your call site doesn't.
...
Four lines:
1. Load `.env` (puts `OPENAI_API_KEY` into `os.environ`)
2. Build a model wrapper bound to a specific GPT-4o model
...
# Swap to "gpt-4o" for the most capable GPT model.
model = ChatOpenAI(model="gpt-4o")
...
python openai/01_model_wrapper_openai.py
```
...
Reads `.env` from the current directory and puts every `KEY=value` into `os.environ`. The OpenAI SDK auto-discovers `OPENAI_API_KEY` from there. Your code never references the secret directly.

### 2. `ChatOpenAI(model="gpt-4o")` — pick a model

The constructor binds:
- **Provider** (OpenAI, implicit in the class name)
- **Model version** (gpt-4o here; gpt-4o for the most capable; gpt-4o-mini for the cheapest)
...
| Pattern | Code shape |
|---|---|
| Provider swap | Change `from langchain_anthropic import ChatAnthropic; model = ChatAnthropic(model="claude-sonnet-4-6")` — no other changes |
...
3. **Swap models** — `ChatOpenAI(model="gpt-4o")` for the smartest, or `gpt-4o-mini` for the cheapest. Note the latency + token-cost differences.
4. **Set `temperature=0`** — `ChatOpenAI(..., temperature=0)`. Re-run twice. Output should be nearly identical (see [lesson 02](02-lcel-composition.md) for the caveats).

---

## Mental model in one line

> **`ChatOpenAI` is an LLM driver. `.invoke(prompt)` is one round-trip to GPT-4o. Every higher-level pattern in this course is composition on top of this single call.**

---

## FAQ

**Q: Why use LangChain at all? Why not just call `openai.OpenAI().chat.completions.create(...)`?**

A: Two reasons. (1) **Composition** — `.invoke()` is a `Runnable`, which means it composes with `|` into chains, parallels into `RunnableParallel`, etc. The raw SDK doesn't give you that. (2) **Provider portability** — switching from GPT-4o to Claude is one-line change. With the raw SDK, you rewrite the call sites. For one-off scripts, raw SDK is fine; for production codebases that may swap providers, the wrapper pays for itself fast.
...
A: Defaults vary by provider; OpenAI defaults to 1.0 (high creativity). **For learning, tests, and structured outputs, set `temperature=0`**. For chat / creative writing, leave it default. For brainstorming, push it to 0.7-1.0. See lesson 02 for the caveats about "deterministic" at temperature=0.

**Q: Where does the model name string come from?**

A: From the provider's model registry. For OpenAI, see the [models page](https://platform.openai.com/docs/models). Common values in this course: `gpt-4o` (balanced), `gpt-4o` (most capable), `gpt-4o-mini` (cheapest/fastest).
...
A: Absolutely. Each `ChatOpenAI(...)` is a separate object. You can have `standard = ChatOpenAI("gpt-4o")` and `mini = ChatOpenAI("gpt-4o-mini")` in the same file and pick which to invoke. This is the foundation of the "model selection per role" pattern in [cost optimization](https://en.wikipedia.org/wiki/Model_selection) (covered later in Phase 1).
