...
| File | Role |
|---|---|
| [`06_parallel_chains_openai.py`](../../openai/06_parallel_chains_openai.py) | Three parallel chains explaining the same topic for different audiences — with timing showing the speedup |
...
The whole `06_parallel_chains_openai.py`:
...
```bash
python openai/06_parallel_chains_openai.py
```
...
3. **Server-side variance** — three concurrent requests to OpenAI land on different GPU pods; under load some serialize.
...
| **Multi-model voting** | Same prompt to GPT-4o + Claude + Gemini, take majority |
...
A: No hard limit, but at ~10+ branches you're rate-limiting OpenAI. Use exponential-backoff retries and consider the OpenAI Batch API for 10+ concurrent requests.
...
A: Not in $ — you make the same N calls either way. Parallel saves *time*. Cheaper happens when the LLM provider also batches your requests, but that's an OpenAI optimization, not LangChain's.
