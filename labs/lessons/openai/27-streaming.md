...
> **Provider variant — OpenAI (`gpt-4o`)** This is the OpenAI version of this lesson. The logic, patterns, and exercises are identical to the Anthropic version. What differs: import is `openai` (raw SDK) and `langchain_openai.ChatOpenAI`, model is `gpt-4o`, and the API key env var is `OPENAI_API_KEY`. Code file: `labs/openai/27_streaming_openai.py`.
...
| File | Role |
|---|---|
| [`27_streaming_openai.py`](../../openai/27_streaming_openai.py) | Four runnable demos: raw SDK streaming with TTFT measurement, LangChain `.stream()` over LCEL, LangGraph stream modes (values/updates/messages), and the tool-use streaming gotcha. |
...
with client.chat.completions.stream(
    ...
    for event in stream:
        if event.type == "content.delta":
            print(event.delta, end="", flush=True)   # render as it arrives
```

The `stream` iterator yields events as they come off the wire. `flush=True` matters — otherwise stdout buffers and you don't actually see the streaming effect.
...
When an agent decides to call a tool, the LLM stream produces a `tool_calls` block and **stops**. The tool then runs (locally — that's a non-LLM operation). Then the agent issues a **second LLM call**, which streams the final answer.
...
2. Detect the `tool_calls` event → render a status indicator: "🌡️ Checking weather in Hyderabad…"
...
```
cd labs
./.venv/bin/python openai/27_streaming_openai.py
```
...
Browser decodes each event by type and renders accordingly. This is what ChatGPT and most production agent UIs actually do.

### Streaming + the cost levers (Session 15)

These compose:
- **Caching**: streaming and prefix caching are independent — the cached_tokens still appear in the streamed usage. Both at once is fine.
- **Model selection**: gpt-4o-mini streams FASTER than gpt-4o (smaller model = lower per-token latency). For perceived-latency-critical paths (welcome message, first answer), gpt-4o-mini is often the right call.
...
- **Batch API**: incompatible with streaming. Batch API is for offline workloads where streaming doesn't help anyway.
...
gpt-4o typically meets the first two. The total-time budget is where retrieval / tool calls / multi-step agents start to matter.
...
1. **Compare gpt-4o vs gpt-4o-mini TTFT.** Swap `MODEL` to `gpt-4o-mini` in Demo 1. Re-run. TTFT typically drops ~30%. For greeting messages or first-impression endpoints, this matters.

2. **Add a long stable system prompt to Demo 1.** Pad the prompt to >1024 tokens and reuse the same prefix across multiple calls. Check `cached_tokens` in the usage object. Does TTFT drop on the second run?
...
The two work together — gpt-4o-mini + streaming + caching gives you sub-300ms TTFT on a chatbot, which is the latency ceiling under which users stop perceiving wait time at all.
...
Because the model still has to do *prefill* — read your prompt, build the KV cache, start generating. The output streams; the input doesn't. With prefix caching, the prefill cost on cache hits drops significantly — TTFT drops correspondingly.
...
Yes — close the stream context. You stop being billed for further tokens. Useful for "stop" buttons in chat UIs.
...
**Q: What's the relationship between `messages` mode (LangGraph) and the raw stream?**
The raw stream is the SDK iterator. `messages` mode is LangGraph wrapping that across all LLM calls in a graph — same per-token deltas, but tagged with which node produced them. For single-LLM-call apps the two are essentially equivalent; for multi-node graphs `messages` is the right abstraction.
...
**Q: How do I render `tool_calls` events in the chat UI?**
