...
| File | Role |
|---|---|
| [`14_multi_agent_openai.py`](../../openai/14_multi_agent_openai.py) | Supervisor + 3 specialist agents (researcher / writer / reviewer) |
| [`14_long_term_memory_openai.py`](../../openai/14_long_term_memory_openai.py) | Agent with conversation memory + vector-stored user facts across sessions |
...
```bash
python openai/14_multi_agent_openai.py
python openai/14_long_term_memory_openai.py
```

`14_multi_agent_openai.py` writes a blog post about prompt caching via supervisor routing. You'll see the supervisor's tool calls printed as it routes: `[supervisor → researcher] ... → writer ... → reviewer ...`.

`14_long_term_memory_openai.py` runs 2 sessions for user "sree". Session 1 facts get saved to LTM. Session 2 (new `thread_id`) starts fresh on short-term memory but recalls Sree's facts from LTM.
...
| You want different models per role (gpt-4o-mini for routing, gpt-4o for writing) | One model handles it all |
...
2. **Make the supervisor use different models per specialist** — gpt-4o-mini for the routing supervisor, gpt-4o for researcher + writer. Watch the cost drop.
3. **Add LTM to `11_production_chatbot_openai.py`** — the capstone has short-term memory but no LTM. Add the two tools and watch the chatbot remember preferences across sessions.
4. **Inspect the LTM after a long conversation** — print all facts saved. See what the agent decided to remember.
