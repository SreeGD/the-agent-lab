...
| File | Role |
|---|---|
| [`05_structured_output_openai.py`](../../openai/05_structured_output_openai.py) | The runnable example — email triage as a typed Pydantic model |
...
python openai/05_structured_output_openai.py
```
...
You never wrote a system prompt. Never said "respond in JSON." Never said "priority must be one of low/medium/high/urgent." GPT-4o *read your Pydantic class* to figure all that out:
...
If GPT-4o tried to return `priority: "extremely-urgent"` (not in the `Literal`), Pydantic raises `ValidationError` *before* `result` is bound. The model **cannot** sneak invalid data into your code path.
...
```
prompt + (tools=[EmailTriage schema])  →  OpenAI API
                                            │
                                            ▼
...
Same propose-execute mechanism as `03_agent_manual_openai.py`'s tool loop — except the "tool" is your data class, not a function.
...
A: OpenAI (GPT-4o+, GPT-4 turbo), Anthropic Claude, Gemini, Bedrock-hosted models, and Mistral support it. Older models and most local models (Llama, Mistral local) need `PydanticOutputParser` instead. Check LangChain's compatibility matrix.

**Q: What happens if GPT-4o returns invalid JSON?**

A: With `with_structured_output`, GPT-4o technically can't — the provider's tool-calling layer enforces schema conformance server-side. What *can* fail is the secondary Pydantic validation (e.g., your custom `field_validator` rejects a combination). In that case Pydantic raises `ValidationError` and you catch and retry, fall back, or surface the error.
...
A: JSON mode just guarantees the response is *valid JSON* — not that it conforms to your schema. You still need a parser and validation. `with_structured_output` is one level higher: it guarantees the response matches *your specific schema*. OpenAI's newer "Structured Outputs" API (distinct from JSON mode) also provides schema enforcement, and `with_structured_output` uses that when available.
