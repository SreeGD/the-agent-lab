...
- Recommend Sonnet for everything because Haiku didn't come to mind
...
```
   ...
   USER ─────────────────►  │   API gateway       │ ◄── auth, rate limit, TLS
                            ...
                            └──────────┬──────────┘
                                       ...
                                       ▼
                            ┌─────────────────────┐
                            │ Orchestrator        │ ◄── intent classifier (Haiku)
   ...
   └────────┬────────┘        └────────┬────────┘      └─────────────────┘
            ...
            └──────────┬───────────────┘
                       ▼
            ┌──────────────────────┐
            │  LLM Gateway         │ ◄── model routing (Haiku/Sonnet),
            │  (caching, retry)    │      cache_control on stable prefix
            └──────────┬───────────┘
                       │
                       ▼
            ┌──────────────────────┐
...
| **Model strategy** | one model / per-role (Haiku triage, Sonnet final) / fine-tune | **Per-role with Haiku for cheap, Sonnet for hard** — Session 15 |
...
With per-role routing: 70% Haiku, 30% Sonnet
Haiku turns:  70M × (500 input + 200 output)
              = 35B input × $0.25/M + 14B output × $1.25/M
              = $8.75K + $17.5K = $26K/month
Sonnet turns: 30M × (800 input + 400 output)
              = 24B input × $3.00/M + 12B output × $15/M
              = $72K + $180K = $252K/month
Total:        ~$278K/month before caching
With prompt caching (60% hit rate on system prompt): saves ~$30K
Net:          ~$248K/month
```
...
- intent classify (Haiku, streamed): 400ms TTFT
- retrieval (vector + BM25): 200ms
- LLM (Sonnet, streamed): 800ms TTFT, 1500ms total for ~300 output tokens
...
If the interviewer says "p95 must be 1 second" — that math doesn't fit. You either drop Sonnet (use Haiku for everything), drop retrieval, or move to a faster region. Knowing the budget line-by-line is what lets you negotiate.
...
▸ LLM model selection strategy
    options: Single Sonnet | Two-tier Haiku+Sonnet | Fine-tuned smaller
    RECOMMEND: Two-tier Haiku for triage, Sonnet for drafting
    because: At 100M turns/month, single Sonnet costs ~$300K/mo;
             two-tier cuts ~60% by routing 70% of turns to Haiku.
...
| Anthropic API outage | L | H | Circuit-breaker, fallback to "we're experiencing issues, escalating to agent" |
...
4. **"What's the most likely thing to break in production?"** — name a real failure mode (Anthropic 5xx, retrieval miss, prompt injection) and describe the detection + response.
...
```
cd labs
./.venv/bin/python 30_system_design_helper.py
```

Takes ~30-60 seconds (three full design generations). Cost ~$0.10 with Sonnet.
...
**Architecture (ASCII):** 16-component diagram showing API gateway → session manager → orchestrator → RAG + tool router + escalation → LLM gateway (Haiku triage + Sonnet drafting) → validator → audit logger.

**Capacity (key numbers, with assumptions):**
- Peak QPS: **~580** (10M MAU × 2 sessions/mo × 5 turns × 15× burst)
- Monthly LLM cost: **~$130K** (after per-role routing + caching)
...
1. LLM model selection → per-role (Haiku triage + Sonnet drafting)
...
- "How would you cut $130K by 50% without degrading UX?"
...
- **Edge / on-device** (private LLMs) — different model selection entirely, no Anthropic API
