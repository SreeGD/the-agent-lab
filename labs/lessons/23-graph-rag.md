...
| File | Role |
|---|---|
| [`23_graph_rag.py`](../23_graph_rag.py) | Extract a knowledge graph from a small focused corpus, build it in NetworkX, run four query demos (list entities by type, 1-hop neighbors, shortest path between two entities, multi-hop QA with subgraph-as-context). |
...
- *"Who at Anthropic worked on the model the AgenticCourse repo uses?"* — requires hopping: AgenticCourse → Claude Sonnet → Claude → Anthropic, plus a person from Anthropic. No single chunk has the full chain.
- *"Which other Anthropic products work with MCP?"* — requires aggregating across chunks
...
class Entity(BaseModel):
    name: str = Field(description="Canonical name, e.g. 'Anthropic'")
...
ORGANIZATION:    Anthropic, LangChain Inc., UKP Lab at TU Darmstadt
PRODUCT:         AgenticCourse, LangChain, LangGraph, Claude Sonnet 4.6,
                 Claude, Claude Agent SDK, Claude Desktop, sentence-transformers
...
```
Q: shortest path Sree Mallipeddi → Anthropic
A: 3-hop path found:
   Sree Mallipeddi --[maintains]--> AgenticCourse
   AgenticCourse --[implements]--> Model Context Protocol
   Model Context Protocol <--[publishes]-- Anthropic
```

The graph found a non-obvious path through MCP. Classical RAG retrieving chunks similar to "Sree Anthropic" would not find this — neither chunk has both names side-by-side.
...
Retrieved subgraph (15 triples within 2 hops of AgenticCourse):
  ...
  AgenticCourse --[uses_as_default_model]--> Claude Sonnet 4.6
  AgenticCourse --[implements]--> Model Context Protocol
  ... [10 more triples]
  Claude Sonnet 4.6 --[member_of]--> Claude
  Anthropic --[builds]--> Claude
  Anthropic --[publishes]--> Model Context Protocol

LLM answer:
  Based on the knowledge graph:
  1. AgenticCourse --[uses_as_default_model]--> Claude Sonnet 4.6
  2. Claude Sonnet 4.6 --[member_of]--> Claude
  3. Anthropic --[builds]--> Claude
  → Anthropic is the company that built the default model.
```

**The LLM traced the three-hop chain explicitly.** That's the GraphRAG superpower — the model never has to "guess" how to connect facts; the graph makes the connection structural.
...
- **Entity resolution / deduplication** — "Anthropic" and "Anthropic, Inc." are different strings; production needs dedup logic (string similarity, embedding clustering, or LLM-based merging).
...
5. **Add entity deduplication** — after extraction, embed entity names and merge any pair with cosine > 0.9. Prevents `Anthropic` and `Anthropic, Inc.` from being separate nodes.
...
A: The system prompt biases the LLM toward canonical names. For production: post-process to dedupe (string similarity, embedding clustering) and to normalize types (you might decide all of "Anthropic Inc.", "Anthropic Co.", "Anthropic Public Benefit Corporation" → "Anthropic").

**Q: What's the cost of extraction?**

A: One LLM call per chunk (or per document). For a corpus of 1000 chunks at ~500 tokens each, you're looking at ~$5-15 of extraction cost (Sonnet 4.6 pricing). Expensive upfront; cheap to query. Re-run only when docs change.
...
A: Yes — combine with Session 9 (native PDF input). Send the PDF as a content block, ask Claude to extract a `KnowledgeGraph`. Same Pydantic schema; the only difference is the input type.
