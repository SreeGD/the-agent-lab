...
| File | Role |
|---|---|
| [`28_production_app.py`](../28_production_app.py) | FastAPI app with `/chat` (SSE streaming), `/health` (liveness + readiness), `/metrics` (Prometheus). Structured JSON logs, request IDs threaded through, manual spans, cost + cache metrics. Includes an in-process demo via FastAPI's `TestClient` so you can run it without binding a port. |
...
```
   ...
   └─────────┘           │  /chat ──► span("chat.llm_stream") ──► │
                         │            anthropic.messages.stream() │  ──► /metrics scraped by
                         │            yield SSE deltas            │      Prometheus / Datadog
                         │                                        │
   ┌─────────┐    GET    │  /health ──► count_tokens (readiness) ─┤  ──► load balancer sees 503
...
@app.get("/health")
async def health():
    # liveness implicit, readiness = "can we reach Anthropic?"
    ...
...
chat_cache_creation_tokens_total 0.0
```
...
**Readiness** — "can it actually serve traffic?" — `count_tokens` exercises auth + reachability cheaply.

Why 503 on readiness failure? Kubernetes / Fly / ECS / Cloud Run all interpret 5xx as "not ready" and pull the pod out of load-balancer rotation. If Anthropic is down, your pod stops getting traffic — automatic graceful degradation. Same code, no extra wiring.
...
```
cd labs
./.venv/bin/python 28_production_app.py
```

Real server (binds port 8000):
```
cd labs
./.venv/bin/python -m uvicorn 28_production_app:app --host 0.0.0.0 --port 8000
```
...
COPY --chown=appuser:appuser 28_production_app.py .
...
CMD ["uvicorn", "28_production_app:app", "--host", "0.0.0.0", "--port", "8000"]
```
...
The `anthropic.Anthropic()` client maintains an httpx connection pool. Default 100 connections — fine for a single replica. Watch:
- One client *per process* (instantiate at import time, not per-request)
- Async variant (`anthropic.AsyncAnthropic`) for async FastAPI routes — uses async httpx, doesn't block the event loop
- The streaming endpoint in this lab is sync but spawned on a thread (FastAPI handles this transparently with `def`); for high concurrency switch to `async def` + `AsyncAnthropic`

### Rate limits + 429 handling

Anthropic rate limits per API key are tier-based. In production:
...
Concurrency limit per replica: Anthropic's rate limit / N replicas. Don't accept more concurrent streams than you can service.
...
1. **Run the real server.** `uvicorn 28_production_app:app --port 8000`. Hit it with `curl -N http://localhost:8000/chat -X POST -d '{"question":"Hi"}'`. See the SSE stream live.

2. **Add the Dockerfile.** Copy the Dockerfile from this lesson, run `docker build -t agenticcourse-chat .` then `docker run -p 8000:8000 -e ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY agenticcourse-chat`. Hit `localhost:8000/health` — same behavior, now containerized.

3. **Deploy to Fly.io.** Get a free account, install `flyctl`, run `fly launch` then `fly secrets set ANTHROPIC_API_KEY=...` then `fly deploy`. ~5 minutes from `git clone` to public URL.
...
**Q: Why is `/health` calling the Anthropic API? Isn't that expensive?**
`count_tokens` is free (no inference). It exercises the auth + reachability path without paying for completion. Cost: ~$0. Latency: ~200-500ms.

**Q: How often is `/health` called?**
k8s default: every 10s. Fly default: every 30s. Cloud Run / ECS: every 30s. Add up to a few thousand calls/day from probes. Free, but if you're calling count_tokens, you might want to cache the readiness result for 5 seconds to reduce noise.
...
The pattern from Session 16 Demo 4: stream the first pass (which ends at the tool_use block), run the tool (async if it's network-bound), then stream the second pass. The client UI renders a status indicator during the gap. For very long tools (>30s), surface progress events: yield `{"type": "tool_progress", "fraction": 0.4}`.
...
- **Session 14 (eval)** — your CI runs `python labs/25_evaluation.py` before deploys; fails the deploy if quality regresses
...
**Q: Where does Anthropic's Console (claude.com/console) fit in?**
The Anthropic Console tracks billing + usage at the account level. Your `chat_cost_usd_total` should *match* what the Console shows (give or take rounding). If they diverge significantly, your cost calculation has a bug — typically a missed cache_read or cache_write field.
...
- **Builds on:** [25 — Evaluation](25-evaluation.md) (the CI quality gate), [26 — Cost Optimization](26-cost-optimization.md) (the metric this app tracks), [27 — Streaming](27-streaming.md) (the chat endpoint pattern), [18 — Anthropic SDK](18-anthropic-sdk.md) (the raw client used here)
