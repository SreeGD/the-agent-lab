"""Memory Architectures — five patterns, one survey lesson (Ollama variant).
...
def count_tokens(messages: list[dict]) -> int:
    """Estimate token count using a char-based estimator (len // 4).

    Note: Anthropic's messages.count_tokens does not exist in Ollama.
    We use a simple char-based estimate: len(text) // 4 as a reasonable
...
def demo_1_working_memory_fifo():
    ...
    print("  Token counting: char-based estimate (len // 4) — Ollama has no")
    print("  native count_tokens API.\n")
