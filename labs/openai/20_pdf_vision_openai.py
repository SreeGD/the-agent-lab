"""Native PDF + image input — OpenAI reads PDFs via rendered PNG pages.
...
at 150 DPI and sent as a vision content block. OpenAI parses layout,
...
def demo_pdf_qa():
    print("\n" + "=" * 70)
    print("DEMO 1 — PDF Q&A (OpenAI reads rendered PDF pages as images)")
    print("=" * 70)
...
def demo_image_read():
    print("\n" + "=" * 70)
    print("DEMO 2 — Image read (OpenAI interprets a chart)")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("PDF + VISION (OpenAI) — PyMuPDF renders pages, gpt-4o reads images")
    print("=" * 70)
    ...
    print("=" * 70)
    print(
        "  • OpenAI does not accept PDF document blocks natively like Anthropic.\n"
        "    Instead, we rasterize each page to PNG (PyMuPDF, 150 DPI) and send\n"
        "    the images in content blocks — gpt-4o reads them via vision.\n\n"
        ...
        "  • Image input is native to gpt-4o — image_url with base64 data URI.\n"
        "    Same pattern for charts, screenshots, diagrams.\n\n"
        "  • Mixed content (PDF pages + images + text) in one call enables\n"
        "    cross-modal reasoning — same capability as Anthropic's native\n"
        "    multimodal, just with an extra rasterization step for PDFs.\n\n"
