...
Run as a service:
    uvicorn 28_production_app:app --host 0.0.0.0 --port 8000

Run the in-process demo (default `python 28_production_app.py`):
...
COST_TOTAL = Counter("chat_cost_usd_total", "Cumulative spend in USD.",
                     registry=REGISTRY)
CACHE_READ_TOTAL = Counter("chat_cache_read_tokens_total",
                           "Cumulative cached input tokens read.",
                           registry=REGISTRY)
CACHE_WRITE_TOTAL = Counter("chat_cache_creation_tokens_total",
                            "Cumulative input tokens cache-written.",
                            registry=REGISTRY)
...
@app.get("/health")
async def health():
    """Liveness (process up) + readiness (Anthropic API reachable)."""
    ...
    try:
        with span("chat.llm_stream", request_id=request_id, question=question[:60]):
            with client.messages.stream(
                model=MODEL,
                max_tokens=200,
                system="Answer concisely. 2-3 sentences max.",
        ...
        COST_TOTAL.inc(cost)
        CACHE_READ_TOTAL.inc(cache_read)
        CACHE_WRITE_TOTAL.inc(cache_write)
...
def run_demo():
    ...
    print("\n" + "=" * 70)
    print("PRODUCTION APP — observability demo via TestClient (in-process)")
    print("=" * 70)
    print("  For real deploys: `uvicorn 28_production_app:app --host 0.0.0.0 --port 8000`")
    ...
    with TestClient(app) as test:
        ...
        wanted = (
            ...
            "chat_cost_usd_total",
            "chat_cache_read_tokens_total", "chat_cache_creation_tokens_total",
        )
...
if __name__ == "__main__":
    ...
    print(
        ...
        "    + total latency histograms, cumulative cost in USD, and\n"
        "    cumulative cache tokens. Grafana + alertmanager turn these\n"
        "    into dashboards + pages.\n\n"
        "  • /health returns 503 if the Anthropic API is unreachable.\n"
